...
poverty_IA <- poverty_IA %>%
  ...
  mutate(log_Minors = log10(Total_Minors), rlog_Minors = round(log_Minors/.5)*.5) 
...
poverty_IA %>%
  ggplot() +
  geom_sf(aes(fill = factor(rlog_Minors))) + 
  labs(title = "Minors Living in Poverty in Iowa") + 
  coord_sf(datum = NA) + 
  theme_minimal() +
  scale_fill_discrete(name = "Count", labels = c("<100", "100-500", "500-1,000", "1,000-5,000", "5,000-10,000", ">10,000")) 
#change Legend to refect actual vaules, exp(log
#Ugly colors...


```
This graph is a good start, but really we want proportions. Get ratios based on total respective age groups.
