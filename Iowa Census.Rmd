...
poverty_IA <- poverty_IA %>% 
  mutate(rper_minors = round(per_minors/5) * 5)
...
poverty_IA %>%
  ggplot() +
  geom_sf(aes(fill = factor(rper_minors))) + 
  labs(title = "Peeercentage of Minors Living in Poverty in Iowa", fill = "Percent Minors") + 
  coord_sf(datum = NA) + 
  theme_minimal() +  
